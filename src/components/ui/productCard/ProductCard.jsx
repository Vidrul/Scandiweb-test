...
class ProductCard extends Component {
  ...
  talkeAtributes(atributes = []) {
    const anyAttributes = [];
    atributes.forEach((a) => anyAttributes.push(a.items[0].id));
    return anyAttributes;
  }

  setAttribute(atributes = []) {
    const defaultAttributes = {};
    if (atributes.length > 0)
      atributes.forEach(
        (item) => (defaultAttributes[item.name] = item.items[0].id)
      );
    ...
    this.props.dispatch(
      addToCart({
        id:
          this.props.id +
          "_" +
          Object.values(this.setAttribute(this.props.atributes)).join("-"),
        atributes:
          this.props.atributes.length > 0 ? this.props.atributes : null,
        name: this.props.name,
        price: this.props.price,
        amount: 1,
        gallery: this.props.gallery,
        brand: this.props.brand,
        selectedAtributeId: this.setAttribute(this.props.atributes),
      })
    );
  }
